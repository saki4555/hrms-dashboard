/**
 * district-move-dialog.jsx
 *
 * Moves a District to a different Region.
 * Cascade: Country (unlocks) → Region
 * Only the final selection (Region) is saved.
 */
...
export default function DistrictMoveDialog({
  open,
  onOpenChange,
  item, // { DISTRICT_ID, DISTRICT_NAME, REGION_ID, COUNTRY_ID }
}) {
  ...
  const currentRegion = regions.find(
    (r) => String(r.REGION_ID) === String(item?.REGION_ID)
  );
  ...
  const handleCountryChange = (value) => {
    ...
    setSelectedRegionId(""); // reset dependent
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success(`"${item.DISTRICT_NAME}" moved successfully!`);
      handleOpenChange(false);
    } catch (error) {
      toast.error(error?.message || "Failed to move district. Please try again.");
    }
  };

  const isUnchanged =
    selectedRegionId &&
    String(selectedRegionId) === String(item?.REGION_ID);
      ...
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              <DialogTitle>Move "{item?.DISTRICT_NAME}"</DialogTitle>
              <DialogDescription>
                Move this district to a different region
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <Alert className="border-amber-500/30 bg-amber-500/5">
            ...
            <AlertDescription className="text-sm text-muted-foreground">
              Changing the region will move this district out of the current list.
            </AlertDescription>
          </Alert>
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium px-3 py-2 bg-muted/50 rounded-md">
              {currentRegion?.REGION_NAME ?? item?.REGION_ID ?? "—"}
              {currentCountry && (
          ...
          <div className="space-y-1.5">
            ...
            {isUnchanged && (
              <p className="text-xs text-amber-500">
                This is already the current region.
              </p>
            )}
          </div>
        </div>
