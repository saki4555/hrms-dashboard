// src\features\settings\work-structure\contract\update-contract-sheet.jsx
...
export default function UpdateContractSheet({ open, onOpenChange, showConfirmation, contract }) {
  ...
  // Populate form when contract data is available
  useEffect(() => {
    if (contract) {
      form.reset({
        ...
        salaryCurrency:        contract.SALARY_CURRENCY || "BDT",
        salaryAmount:          contract.SALARY_AMOUNT ?? "",
        probationPeriodMonths: contract.PROBATION_PERIOD_MONTHS ?? "",
      ...
      if (contract.EMPLOYEE_ID) {
        setSelectedEmployee({
          ...
          name:  [contract.FIRST_NAME, contract.LAST_NAME].filter(Boolean).join(" ") || `Employee #${contract.EMPLOYEE_ID}`,
          empNo: contract.EMP_NO || "",
        });
      }
    }
  }, [contract, form]);
  ...
  // isDirty guard prevents overwriting the existing end date on initial load
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await updateContractMutation.mutateAsync({ id: contract.CONTRACT_ID, data: backendData });
      toast.success("Contract updated successfully!");
      form.reset();
      onOpenChange(false);
    } catch (error) {
      toast.error(error?.message || "Failed to update contract. Please try again.");
    }
  };
        ...
        <SheetHeader className="px-6 py-5 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <SheetDescription>
                Edit contract for "{[contract?.FIRST_NAME, contract?.LAST_NAME].filter(Boolean).join(" ") || `Employee #${contract?.EMPLOYEE_ID}`}"
              </SheetDescription>
            </div>
          </div>
        </SheetHeader>
          ...
          >
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-border shrink-0">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? <><Spinner className="mr-2 h-4 w-4" />Updating...</> : "Update Contract"}
              </Button>
            </div>
          </form>
        </Form>
