...
import { RefreshCw, AlertCircle, Clock, ArrowUp, ArrowDown, ChevronDown, CheckCircle2, XCircle, AlertTriangle, LogOut, CalendarIcon } from "lucide-react";
...
import { useTeamAttendance,  } from "./queries"; 
import AttendanceDetailDialog from "./attendance-detail-dialog";
import { Badge } from "@/components/ui/badge";
import { Calendar } from "@/components/ui/calendar";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { useAuthV2 as useAuth } from "../authentication-v2/use-auth-v2"; 

// Re-use the same helpers and STATUS_CONFIG from the main attendance page
...
const SORT_COLUMN_MAP    = { employee: "FIRST_NAME", ATTENDANCE_DATE: "ATTENDANCE_DATE" };
const REVERSE_SORT_MAP   = { FIRST_NAME: "employee", ATTENDANCE_DATE: "ATTENDANCE_DATE" };
...
function SortHeader({ column, children, className }) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className={cn("-ml-1.5 flex h-8 items-center gap-1.5 rounded-md px-2 py-1.5 hover:bg-accent focus:outline-none [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:text-muted-foreground", className)}>
        {children}
        {column.getCanSort() && (
          column.getIsSorted() === "desc" ? <ArrowDown /> :
          column.getIsSorted() === "asc"  ? <ArrowUp />   : <IconSelector />
        )}
      </DropdownMenuTrigger>
...
export default function AttendanceTeam() {
  const { user } = useAuth();                         // employee_id comes from here
  ...
  useEffect(() => {
    if (!fromDate && !toDate) { setFromDate(today); setToDate(today); }
  }, []); // eslint-disable-line
  ...
  const { data: response, isLoading, isError, error, refetch, isFetching } =
    useTeamAttendance(supervisorId, backendParams);
  ...
  const sorting = useMemo(() => {
    if (!sortBy) return [];
    return [{ id: REVERSE_SORT_MAP[sortBy] ?? sortBy, desc: sortOrder === "DESC" }];
  }, [sortBy, sortOrder]);
        ...
        return (
          <div className="flex items-center gap-3 py-1">
            <Avatar className="h-8 w-8 shrink-0">
              <AvatarImage src={`${import.meta.env.VITE_API_BASE_URL}/api/emp-images/person/${r.EMPLOYEE_ID}`} />
              <AvatarFallback className={cn("text-xs font-semibold text-white", getAvatarColor(fullName))}>{initials}</AvatarFallback>
            </Avatar>
  ...
  if (isLoading) return <div><PageHeader /><div className="bg-card rounded-md shadow-sm p-4"><div className="flex flex-col items-center justify-center py-16"><Spinner className="h-12 w-12 mb-4" /><p className="text-muted-foreground">Loading team attendance...</p></div></div></div>;
    ...
    <div>
      ...
      <div className="bg-card rounded-md shadow-sm p-4">
        <div className="space-y-4">
          ...
          {/* Filter bar — no employee picker, supervisor sees only their team */}
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(status || fromDate || toDate) && (
              <Button variant="ghost" size="sm" className="h-9 border border-dashed text-muted-foreground"
                onClick={() => { setFromDate(null); setToDate(null); setStatus(null); setPage(1); }}>
                ...
                {rows.length ? table.getRowModel().rows.map((row) => (
                  ...
                  <TableRow>
                    <TableCell colSpan={columns.length} className="h-32 text-center">
                      <Empty><EmptyHeader><EmptyMedia variant="icon"><Clock /></EmptyMedia><EmptyTitle>No Team Attendance Found</EmptyTitle></EmptyHeader></Empty>
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
    ...
    <div className="bg-card rounded-md shadow-sm p-4 mb-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-0.5">
          <h1 className="text-lg md:text-2xl font-semibold tracking-tight">My Team's Attendance</h1>
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Team Attendance</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        {onRefetch && (
