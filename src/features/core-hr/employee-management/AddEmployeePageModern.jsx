      ...
      <div className="max-w-5xl mx-auto px-6 py-4">
        ...
        <div className="mt-3 flex items-center gap-3">
          <div className="flex-1 h-1 bg-muted rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-violet-500 via-blue-500 to-emerald-500 transition-all duration-700 rounded-full"
              style={{ width: `${(completedSteps.size / (STEPS.length - 1)) * 100}%` }}
            />
          </div>
          <span className="text-xs text-muted-foreground whitespace-nowrap">
            {completedSteps.size}/{STEPS.length - 1} sections done
          </span>
        </div>
      </div>
    </div>
...
function LiveSummary({ form, currentStep }) {
  ...
  return (
    <div className="hidden xl:block w-64 shrink-0">
      <div className="sticky top-36 rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden">
        <div className="px-4 py-3 bg-gradient-to-r from-violet-500/10 to-blue-500/10 border-b border-border/50">
          <div className="flex items-center gap-2">
            <Sparkles className="w-3.5 h-3.5 text-violet-500" />
            <span className="text-xs font-semibold text-foreground uppercase tracking-wider">Live Preview</span>
          </div>
        </div>
        <div className="p-4 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold truncate">{fullName || "New Employee"}</p>
              <p className="text-xs text-muted-foreground">{empNo || "No ID yet"}</p>
            </div>
          </div>
          <div className="h-px bg-border/50" />
          {[
            { label: "Gender", value: gender || "—" },
            { label: "Join Date", value: joinDate ? format(joinDate, "dd MMM yyyy") : "—" },
            { label: "Nationality", value: nationality || "—" },
          ].map(({ label, value }) => (
            <div key={label} className="flex justify-between items-start gap-2">
              <span className="text-xs text-muted-foreground shrink-0">{label}</span>
              <span className="text-xs font-medium text-right truncate max-w-[120px]">{value}</span>
            </div>
          ))}
        </div>
        <div className="px-4 pb-4">
          ...
          {STEPS.slice(0, -1).map((step, idx) => {
            ...
            const isActive = idx === currentStep;
            return (
              <div key={step.id} className={cn("flex items-center gap-2 py-1 text-xs rounded px-1", isActive && "bg-primary/5")}>
                <div className={cn(
                  "w-4 h-4 rounded-full flex items-center justify-center shrink-0",
                  isDone ? "bg-emerald-500/20 text-emerald-500" : isActive ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground",
                )}>
                  {isDone ? <Check className="w-2.5 h-2.5" /> : <Icon className="w-2.5 h-2.5" />}
                </div>
                <span className={cn(
                  isDone ? "text-emerald-500 line-through opacity-60" : isActive ? "text-foreground font-medium" : "text-muted-foreground",
                )}>
                  {step.label}
                </span>
              </div>
...
function SmartRadioGroup({ form, name, label, options }) {
  return (
    <FormField
      control={form.control}
      name={name}
  ...
  const groups = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: "Effective Dates", value: v.effectiveStartDate ? `${format(v.effectiveStartDate, "dd MMM yyyy")} → ${format(v.effectiveEndDate, "dd MMM yyyy")}` : "—" },
      ],
    },
    {
      title: "Assignment",
...
// ─── Position Combobox ────────────────────────────────────────────────────────
function PositionComboboxField({ form, filteredPositions, selectedOrgId, orgPositionsLoading, grades }) {
  const [posOpen, setPosOpen] = useState(false);
  const fieldValue = form.watch("positionId");
  const isDisabled = !selectedOrgId || orgPositionsLoading;
...
export default function AddEmployeePageModern() {
  ...
  useEffect(() => {
    if (effectiveStart) form.setValue("effectiveEndDate", addYears(effectiveStart, 5), { shouldDirty: true });
  }, [effectiveStart]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (assignmentStart) form.setValue("assignmentEffectiveEndDate", addYears(assignmentStart, 5), { shouldDirty: true });
  }, [assignmentStart]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (presentStart) form.setValue("presentAddress.effectiveEndDate", addYears(presentStart, 5), { shouldDirty: true });
  }, [presentStart]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (permanentStart) form.setValue("permanentAddress.effectiveEndDate", addYears(permanentStart, 5), { shouldDirty: true });
  ...
  // Count filled fields per step — memoized for all steps at once
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    // Watch all relevant fields so progress updates live
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setStatusMessage(error?.message || "Failed to create employee. Please try again.");
    }
  };
  ...
  // ─── All Steps Rendered, visibility controlled by CSS ────────────────────
  // KEY FIX: all steps are always mounted so RHF never unregisters any field.
  // Hidden steps use `display: none` via the `hidden` class.
  return (
      ...
      <header className="px-6 mb-6 pt-2">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <h1 className="text-xl md:text-2xl font-bold tracking-tight">New Employee Onboarding</h1>
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage className="text-muted-foreground/70 font-normal">Add Employee</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, handleFormError)}>
          <div className="flex gap-6 px-6 mt-6">
            {/* ── Step Content — all steps always mounted ── */}
            <div className="flex-1 min-w-0">
              ...
              <div className={cn(currentStep !== 0 && "hidden")}>
                ...
                >
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FieldWithCounter form={form} name="firstName" label="First Name" placeholder="Enter first name" maxLength={50} />
                      <FieldWithCounter form={form} name="lastName" label="Last Name" placeholder="Enter last name" maxLength={50} />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FieldWithCounter form={form} name="fathersName" label="Father's Name" placeholder="Enter father's name" maxLength={100} />
                      <FieldWithCounter form={form} name="fathersNameB" label="Father's Name (Bangla)" placeholder="পিতার নাম" maxLength={100} />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FieldWithCounter form={form} name="mothersName" label="Mother's Name" placeholder="Enter mother's name" maxLength={100} />
                      <FieldWithCounter form={form} name="mothersNameB" label="Mother's Name (Bangla)" placeholder="মাতার নাম" maxLength={100} />
                    </div>
                      ...
                      <FormField control={form.control} name="dateOfBirth"
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <DatePicker value={field.value} onChange={field.onChange} placeholder="Select date of birth" />
                            </FormControl>
                            <FormMessage className="text-xs" />
                          </FormItem>
                        )}
                      />
                      <FieldWithCounter form={form} name="nid" label="NID" placeholder="Enter NID number" maxLength={30} />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FieldWithCounter form={form} name="birthRegNo" label="Birth Registration No" placeholder="Enter birth reg. no" maxLength={30} />
                      <FieldWithCounter form={form} name="townOfBirth" label="Town of Birth" placeholder="Enter town of birth" maxLength={30} />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FieldWithCounter form={form} name="regionOfBirth" label="Region of Birth" placeholder="Enter region" maxLength={30} />
                      <FieldWithCounter form={form} name="countryOfBirth" label="Country of Birth" placeholder="Enter country" maxLength={30} />
                    </div>

                    <SmartRadioGroup form={form} name="maritalStatus" label="Marital Status"
                      options={MARITAL_STATUS_OPTIONS} />

                    <FieldWithCounter form={form} name="nationality" label="Nationality" placeholder="Enter nationality" maxLength={30} />
                  </div>
                </SectionCard>
              </div>
              ...
              <div className={cn(currentStep !== 1 && "hidden")}>
                ...
                >
                  <div className="space-y-5">
                    <FieldWithCounter form={form} name="empNo" label="Employee Number" placeholder="Enter employee number" maxLength={20} />
                      ...
                      <FormField control={form.control} name="joinDate"
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <DatePicker value={field.value} onChange={field.onChange} placeholder="Select join date" />
                            </FormControl>
                            <FormMessage className="text-xs" />
                          </FormItem>
                        )}
                      ...
                      <FormField control={form.control} name="effectiveStartDate"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-sm font-medium">Effective Start Date *</FormLabel>
                            <FormControl>
                              <DatePicker value={field.value} onChange={field.onChange} placeholder="Select effective start date" />
                            </FormControl>
                            <FormMessage className="text-xs" />
                          </FormItem>
                        )}
                    ...
                    <FormField control={form.control} name="effectiveEndDate"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-sm font-medium">Effective End Date *</FormLabel>
                          <FormControl>
                            <DatePicker value={field.value} onChange={field.onChange} placeholder="Select effective end date" />
                          </FormControl>
                          <FormMessage className="text-xs" />
                        </FormItem>
                      )}
                    ...
                    <FormField control={form.control} name="personTypeId"
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} value={field.value} disabled={personTypesLoading}>
                            <FormControl>
                ...
                >
                  <div className="space-y-5">
                    <ComboboxField form={form} name="companyId" label="Company" items={companies}
                      ...
                      placeholder={companiesLoading ? "Loading..." : "Select company"} disabled={companiesLoading} />

                    <ComboboxField form={form} name="ouId" label="Operational Unit" items={organizations}
                      idKey="ID" nameKey="NAME"
                      placeholder={organizationsLoading ? "Loading..." : "Select operational unit"} disabled={organizationsLoading} />

                    <ComboboxField form={form} name="orgId" label="Organization" items={organizations}
                      idKey="ID" nameKey="NAME"
                      placeholder={organizationsLoading ? "Loading..." : "Select organization"} disabled={organizationsLoading}
                      onSelect={(org) => {
              ...
              <div className={cn(currentStep !== 3 && "hidden")}>
                <div className="space-y-5">
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-xl border border-border/60 bg-card">
                    <Switch id="same-address" checked={sameAsPresent} onCheckedChange={handleSameAsPresent} />
                    <div>
                  ...
                  <SectionCard
                    ...
                    filledCount={0}
                  >
                    <AddressFields form={form} prefix="permanentAddress" disabled={sameAsPresent} />
                  </SectionCard>
                </div>
              </div>
            ...
            {/* Live Summary Sidebar */}
            <LiveSummary form={form} currentStep={currentStep} />
          </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {submitStatus === "success" && newPersonId && (
                  <div className="flex items-center gap-2 text-emerald-600 text-sm">
                    ...
                    <Link to={`/core-hr/employee-management/employee-details/${newPersonId}`} className="underline font-medium">View Details</Link>
                  </div>
                )}
                {!submitStatus && (
