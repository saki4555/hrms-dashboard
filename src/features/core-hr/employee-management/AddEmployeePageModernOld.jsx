              ...
              <React.Fragment key={step.id}>
                ...
                {idx < STEPS.length - 1 && (
                  <div className="flex-1 h-0.5 relative overflow-hidden rounded-full bg-border">
                    <div
                      className={cn(
                        "absolute inset-y-0 left-0 bg-gradient-to-r from-emerald-400 to-emerald-500 transition-all duration-500 rounded-full",
                      )}
    ...
    <div className="hidden xl:block w-64 shrink-0">
      <div className="sticky top-36 rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          {summaryItems.slice(2).map(({ label, value }) => (
            <div key={label} className="flex justify-between items-start gap-2">
              <span className="text-xs text-muted-foreground shrink-0">
                {label}
              </span>
              <span className="text-xs font-medium text-right truncate max-w-[120px]">
                {value}
              </span>
            </div>
          ))}
          ...
          {STEPS.slice(0, -1).map((step, idx) => {
            ...
            return (
              <div
                key={step.id}
                className={cn(
                  "flex items-center gap-2 py-1 text-xs rounded px-1",
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {options.map((opt) => {
              const val = String(opt.value ?? opt); // ← always a string
              const lbl = opt.label ?? opt;
              ...
              return (
                <button
                  ...
                  onBlur={field.onBlur} // ← moved off onClick, onto onBlur
                  className={cn(
                    "px-4 py-2 rounded-lg border text-sm font-medium transition-all duration-200 cursor-pointer",
                    isSelected
                      ? "bg-primary text-primary-foreground border-primary shadow-sm shadow-primary/30"
              ...
              <FormControl>
                <div className="relative">
                  <Input
                    ...
                    disabled={disabled}
                    {...field}
                    className={fieldClass(fieldState)}
                  />
                  {field.value && !fieldState.error && (
                    <Check className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-emerald-500" />
                  )}
...
function SectionCard({
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card overflow-hidden shadow-sm">
      <div
        className={cn(
          "px-6 py-4 flex items-center justify-between bg-gradient-to-r opacity-90",
          gradient,
        )}
      >
        <div className="flex items-center gap-3">
  ...
  const groups = [
    {
      ...
      items: [
        ...
        {
          label: "Date of Birth",
          value: v.dateOfBirth ? format(v.dateOfBirth, "dd MMM yyyy") : "—",
        },
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: "Join Date",
          value: v.joinDate ? format(v.joinDate, "dd MMM yyyy") : "—",
        },
        {
          label: "Person Type",
          value:
            personTypes.find((p) => String(p.PERSON_TYPE_ID) === v.personTypeId)
              ?.PERSON_TYPE || "—",
        },
        {
          label: "Registered Disability",
          value: v.regDisability === "1" ? "Yes" : "No",
        },
        {
      ...
      items: [
        {
          label: "Company",
          value:
            companies.find((c) => String(c.COMPANY_ID) === v.companyId)
              ?.COMPANY_NAME || "—",
        },
        {
          label: "Organization",
          value:
            organizations.find((o) => String(o.ID) === v.orgId)?.NAME || "—",
        },
        { label: "Payroll ID", value: v.payrollId },
        {
          label: "Grade",
          value: grades.find((g) => String(g.ID) === v.gradeId)?.GRADE || "—",
        },
      ],
    },
    {
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Please review the information below before creating the employee.
          </p>
        </div>
      </div>
...
// ─── Position Combobox (extracted to avoid hook-in-render-prop violation) ─────
function PositionComboboxField({
  form,
  filteredPositions,
  selectedOrgId,
  orgPositionsLoading,
  ...
  const selectedPosition = filteredPositions.find(
    (pos) => String(pos.POSITION_ID) === String(fieldValue),
  );
                    ...
                    {filteredPositions.map((pos) => {
                      ...
                      return (
                        <CommandItem
                          ...
                          onSelect={() => {
                            if (!isFull)
                              handlePositionSelect(pos, field.onChange);
                          }}
                          className={
                        ...
                        >
                          ...
                          {isFull ? (
                            ...
                            <Check
                              className={cn(
                                "ml-auto h-4 w-4",
                                String(field.value) === String(pos.POSITION_ID)
                                  ? "opacity-100"
                                  : "opacity-0",
                              )}
                            />
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function AddEmployeePageModernOld() {
  ...
  // Auto-update end dates — watch each field individually to avoid new-array-ref on every render
      ...
      case 0:
        return (
          <SectionCard
            ...
            filledCount={stepProgress.filled}
          >
            <div className="space-y-5">
              <SmartRadioGroup
                form={form}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger
                          className={cn(field.value && "border-emerald-500/50")}
                        >
                          <SelectValue
                            placeholder={
                              personTypesLoading
    ...
    <PageContainer className="px-0 pb-32">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, handleFormError)}>
          ...
          {/* Validation error summary (above action bar) */}
